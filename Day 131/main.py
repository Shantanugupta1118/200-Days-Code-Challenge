...
import sys
file2 = "Output/f_find_great_mentors.txt"
...
# def getLevelContributor(project)
        ...
        for contributor in contributors:
            for role in project.roles:
                if contributor.canFulfillRole(role):
                    if asg.get(role) != None:
                        if asg[role].LevelContri(role) > contributor.LevelContri(role):
                            asg[role] = contributor
                    else:
                        asg[role] = contributor
                    break

        if len(asg) == len(project.roles):
